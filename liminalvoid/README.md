...
- If no model file exists, `predict()` returns a normal room as fallback. Call `POST /train` once before starting a session to generate the model weights.
...
The synthetic data design required explicit thought about what each anomaly type actually looks like as behaviour. A void player has near-zero velocity, high dwell time, and maximum wall proximity. A loop player has high backtrack count. These archetypes were designed manually before a single line of training code was written — the ML problem only became tractable once the behavioural signatures were clearly defined.

## Current Constraints and Next Steps

- The model is trained on synthetic archetypes, not real captured playtest sessions yet.
- The transformer is trained on synthetic archetypes. Accuracy on real player behaviour is untested.
...
- The room system can grow into more biome variation, audio cues, and persistent narrative state.
- Model weights reload from disk on every prediction call. A module-level singleton would eliminate this latency.
...
The major pieces are present in one codebase:
